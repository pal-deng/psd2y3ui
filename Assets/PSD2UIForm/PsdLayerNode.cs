    ...
    public class PsdLayerNode : MonoBehaviour
    {
        ...
        internal bool ParseTextLayerInfo(out string text, out int fontSize, out float characterSpace, out float lineSpace, out Color fontColor, out UnityEngine.FontStyle fontStyle, out TMPro.FontStyles tmpFontStyle, out string fontName)
        {
            ...
            if (IsTextLayer(out var txtLayer))
            {
                ...
                try
                {
                    r = txtLayer.TextColor.R;
                }
                catch (Exception e)
                {
                    Debug.Log("r parse failed" + e.Message);
                }

                try
                {
                    g = txtLayer.TextColor.G;
                }
                catch (Exception e)
                {
                    Debug.Log("g parse failed" + e.Message);
                }

                try
                {
                    b = txtLayer.TextColor.B;
                }
                catch (Exception e)
                {
                    Debug.Log("b parse failed" + e.Message);
                }

                try
                {
                   var color = txtLayer.TextColor;
                }
                catch (Exception e)
                {
                    Debug.Log("color parse failed" + e.Message);
                }
                ...
                {
                    Debug.Log("a parse failed" + e.Message);
                }
